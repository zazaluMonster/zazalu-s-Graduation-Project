...
$(document).ready(function () {
    ...
    $(".province").on("select2:select", function () {
        console.log($("#select2-addressProvince-container").text());
        //刷新的列表
        if($("#addressCity").find($('option').length) !== 0){
            $("#addressCity option").remove();
        }
        //获取到修改了的地址 生成其二级地址
        $.ajax({
    ...
    $(".city").on("select2:select", function () {
        console.log($("#select2-addressCity-container").text());
        if($("#addressArea").find($('option').length) !== 0){
            $("#addressArea option").remove();
        }
        //获取到修改了的地址 生成其二级地址
        $.ajax({
            url: '/MyChannel/JSP/json/chinaMapList.json',
    ...
    function succcityFunction(tt) {
        var json = eval(tt); //数组
        var tt = "";
        var selectProvince = $("#select2-addressProvince-container").text();
        ...
        //删除加载中
        if ($("#addressCityJiaZai") !== undefined){
            $("#addressCityJiaZai").remove();
            console.log("删除加载中")
        }
        //重新列表化省份
        $(".city").select2();
    ...
    function succareaFunction(tt) {
        var json = eval(tt); //数组
        var tt = "";
        var selectCity = $("#select2-addressCity-container").text();
        var selectCityId;
        $.each(json.result[1] , function (index,item) {
        ...
        //删除加载中
        if ($("#addressAreaJiaZai") !== undefined){
            $("#addressAreaJiaZai").remove();
            console.log("删除加载中")
        }
        //重新列表化省份
        $(".area").select2();
